...
func JudgeMent_OneUrl_OneJson(url string, oneExpjson Configs.ExpJson, timeout_count map[string]int) bool {
	var paras params
	// 将Exp中的通配符替换为命令
	...
	for key, value := range oneExpjson.Request {
		...
		if value.Upload.FilePath == "" {
			RespObject.Resp = Client(value.Method, whole_url, ReqData, value.Header, 5, value.Follow_redirects, timeout_count, paras) //最后一个url为原生url，用来统计是否不继续对该url发包
		} else {
			RespObject.Resp = UClient(value.Method, whole_url, filestruct, ReqData, value.Header, 5, value.Follow_redirects, timeout_count, paras)
		}
