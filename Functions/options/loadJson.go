...
func LoadExpJson(filepath string) {
	...
	if err != nil {
		MistakPrint("open " + filepath + " failed")
		os.Exit(1)
	}
	fileValue, err := ioutil.ReadAll(file)
	if err != nil {
		MistakPrint("read " + filepath + " body failed")
		os.Exit(1)
	}
	err = json.Unmarshal(fileValue, &Configs.ExpJsonMap)
	if err != nil {
		MistakPrint("Json file to load failed")
		os.Exit(1)
	}
}
...
func LoadOneExpJson(filepath string, oneExpjson *Configs.ExpJson) {
	file, err := os.Open(filepath)
	if err != nil {
		MistakPrint(" LoadOneExpJson func open " + filepath + " failed")
		fmt.Println("LoadOneExpJson func open filepath err...")
	}
	fileValue, err := ioutil.ReadAll(file)
	if err != nil {
		MistakPrint("LoadOneExpJson  read " + filepath + " body failed")
		fmt.Println("LoadOneExpJson func readAll err...")
	}
	err = json.Unmarshal(fileValue, oneExpjson)
	if err != nil {
		MistakPrint("LoadOneExpJson Json file to load failed")
